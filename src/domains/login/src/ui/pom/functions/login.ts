...
export const login = async (
    ...
    await page.route('http://localhost:3000/login', async route => {
        ...
        // Optional: validate request against contract
        // expect(request).toMatchObject(
        //     loginContract.validUser.request
        // );

        // await route.fulfill({
        //     status: loginContract.validUser.response.status,
        //     contentType: 'application/json',
        //     body: JSON.stringify(
        //         loginContract.validUser.response.body
        //     )
        // });
    });
